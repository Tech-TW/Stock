...
with st.sidebar:
    st.header("Settings")
    if st.button("🗑️ 清除快取 (Clear Cache)", help="若股價未更新或資料異常，請點擊此處重抓"):
        st.cache_data.clear()
...
def download_fx_history(currency: str, start_date: pd.Timestamp, end_date: pd.Timestamp) -> pd.DataFrame:
    ...
    try:
        ...
        # download progress=False 以保持 UI 乾淨
        fx = yf.download(fx_symbol, start=start_ex, end=end_ex, auto_adjust=True, progress=False)
        if fx.empty:
            return pd.DataFrame()
        
        # 處理 MultiIndex 或是單層 Index
        if isinstance(fx.columns, pd.MultiIndex):
            fx_df = fx["Close"].reset_index()
            # 如果 reset_index 後 columns 仍有層級問題，嘗試扁平化
            if isinstance(fx_df.columns, pd.MultiIndex):
                fx_df.columns = [c[0] if isinstance(c, tuple) else c for c in fx_df.columns]
        else:
            fx_df = fx["Close"].reset_index()

        # 確保欄位名稱正確
        if "Date" in fx_df.columns:
            fx_df = fx_df.rename(columns={"Date": "日期"})
        elif "index" in fx_df.columns: # 有時 reset_index 會變成 index
             fx_df = fx_df.rename(columns={"index": "日期"})
             
        # 尋找 Close 欄位 (yfinance 有時會回傳 ticker 名稱作為欄位)
        # 簡單處理：除了日期以外的第一個欄位視為匯率
        cols = [c for c in fx_df.columns if c != "日期"]
        if cols:
             fx_df = fx_df.rename(columns={cols[0]: "匯率"})
        else:
            return pd.DataFrame()
        ...
        fx_df["幣別"] = currency
        return fx_df[["日期", "匯率", "幣別"]]
    except Exception as e:
        print(f"FX Download Error ({currency}): {e}")
        return pd.DataFrame()
...
def get_fx_rate(date: pd.Timestamp, currency: str, fx_data_dict: dict) -> float:
    ...
    if f is None or f.empty: return np.nan
    ...
    try:
        ...
        if isinstance(s.columns, pd.MultiIndex):
            ...
            if isinstance(sdf.columns, pd.MultiIndex):
                 sdf.columns = [c[0] if isinstance(c, tuple) else c for c in sdf.columns]
        else:
            sdf = s["Close"].reset_index()

        if "Date" in sdf.columns:
            sdf = sdf.rename(columns={"Date": "日期"})
        elif "index" in sdf.columns:
             sdf = sdf.rename(columns={"index": "日期"})

        cols = [c for c in sdf.columns if c != "日期"]
        if cols:
             sdf = sdf.rename(columns={cols[0]: "收盤價"})
        else:
            return pd.DataFrame()
        ...
        sdf["股票代號"] = ticker
        return sdf[["日期", "收盤價", "股票代號"]]
    except Exception as e:
        print(f"Stock Download Error ({ticker}): {e}")
        return pd.DataFrame()
...
class StockEventProcessor:
    ...
    def _calculate_shares_before_date(self, transaction_history: pd.DataFrame, ticker, target_date):
        t = transaction_history[
            (transaction_history["股票代號"]==ticker) & (transaction_history["日期"]<target_date)
        ]
    ...
    def apply_stock_split_with_timing(self, position_data, ticker, split_date, split_ratio, transaction_history=None):
        if ticker not in position_data: return None
    ...
    for _, row in df_trades.sort_values("日期").iterrows():
        ...
        if shares > 0:
            ...
            while qty_to_sell > 1e-12 and fifo_positions[ticker]:
                ...
                realized += pnl_foreign * lot["fx"] # 這裡簡化用買入時匯率或可改用賣出時匯率，此處維持邏輯一致性
...
def run_full_analysis(trades_df: pd.DataFrame, valuation_to_today: bool = True) -> dict:
    ...
    if "幣別" not in df.columns: df["幣別"] = df["股票代號"].apply(determine_currency)
    ...
    for _, row in df_trades.sort_values("日期").iterrows():
        ...
        if tkr not in position_data:
            position_data[tkr] = {"shares":0.0,"avg_cost_foreign":0.0,"pure_cost_foreign_total":0.0, "avg_exchange_rate":0.0,"total_cost_twd":0.0,"currency":cur}
    ...
    for ticker, g in df_trades.sort_values("日期").groupby("股票代號"):
        ...
        pool_shares = 0.0; pool_avg_cost_foreign = 0.0; pool_total_cost_foreign = 0.0
        pool_total_cost_twd = 0.0; pool_avg_fx = 1.0
        buy_lots = []

        for idx, row in g.iterrows():
            shares = float(row["購買股數"]); price = float(row["購買股價"])
            fee = float(row.get("交易成本", 0.0)); fx = float(row.get("換匯匯率", 1.0))
            ...
            if shares > 0:
                ...
                sell_qty = -shares
                if sell_qty<=0 or pool_shares<=0: continue
                ...
                if pool_shares <= 1e-8:
                    pool_shares = 0.0; pool_total_cost_foreign = 0.0; pool_total_cost_twd = 0.0
                    pool_avg_cost_foreign = 0.0; pool_avg_fx = 1.0
                else:
    ...
    # 10) 投組每日總彙整（到估值日）
    with st.spinner("Calculating Daily Portfolio Evolution..."):
        ...
        # 這裡雖然是迴圈，但主要是為了正確累積「部位」變化
        # 價格查找已改為從預處理好的 Series 取值 (get)
        for day in all_dates:
            if day in trades_by_day:
                for _, r in trades_by_day[day].iterrows():
                    ...
                    if tkr not in positions:
                        positions[tkr] = {"shares":0.0,"avg_cost_foreign":0.0,"pure_cost_foreign_total":0.0, "avg_fx":0.0,"total_cost_twd":0.0,"currency":ccy}
                    p=positions[tkr]
            ...
            for tkr,p in positions.items():
                ...
                ccy = p["currency"]
                # 從預先填充的 Series 取值，避免重複 DataFrame 查詢
                px_today = stock_close_daily.get(tkr, {}).get(day, np.nan)
                fx_today = fx_daily.get(ccy, {}).get(day, np.nan)
    ...
    if not daily_portfolio_df.empty:
        daily_portfolio_df = daily_portfolio_df.rename(columns={"總權益(台幣)":"投組總額_日報"})
    ...
    if not daily_portfolio_df.empty:
        ...
        monthly_eom_df["月底"] = monthly_eom_df["月底"].dt.strftime('%Y-%m') # 格式化日期
    else:
    ...
    try:
        ...
        if not daily_portfolio_df.empty:
            ...
            figs["equity_curve"] = fig_equity
        
        # 2. 月底市值長條圖
        if not monthly_eom_df.empty:
            # 轉換長格式以便分組繪圖 (總成本 vs 總市值)
            df_melt = monthly_eom_df.melt(id_vars=["月底"], value_vars=["總成本(台幣)", "總市值(台幣)"], var_name="類別", value_name="金額")
            fig_monthly = px.bar(
                df_melt, x="月底", y="金額", color="類別", barmode="group",
                title="每月月底資產概況 (成本 vs 市值)",
                text_auto='.2s'
            )
            figs["monthly_bar"] = fig_monthly
    ...
    dataframes = {
        ...
        "costs": cost_df,
        "daily_equity": daily_portfolio_df[["日期","投組總額_日報"]] if not daily_portfolio_df.empty else pd.DataFrame(),
...
if result:
    ...
    with tabs[7]:
        ...
        # 優先顯示圖表
        if figs.get("monthly_bar") is not None:
            st.plotly_chart(figs["monthly_bar"], use_container_width=True)
